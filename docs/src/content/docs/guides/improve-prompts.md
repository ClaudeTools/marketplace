...
> "/prompt-improver plan refactor the authentication middleware to support OAuth2"
...
> "/prompt-improver task build a real-time notification system with WebSocket support"
...
- **Clear task, just do it**: Skip `/prompt-improver` entirely — just tell Claude what you want
