...
<Tabs>
  ...
  <TabItem label="Python">
    You have a function called `get_user` in `services/user_service.py`. It does more than fetch — it also validates and enriches — so you want to rename it to `resolve_user` across the codebase. It's used in 6 places. You want to be sure every caller gets updated and nothing is accidentally left behind.
  </TabItem>
</Tabs>
...
- [Tutorial: Your First Bug Fix](tutorial-first-bug-fix.mdx) — apply the same read-first discipline to debugging
