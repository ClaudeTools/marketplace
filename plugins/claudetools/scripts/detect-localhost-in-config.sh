#!/bin/bash
# PostToolUse:Edit|Write hook — warns when localhost URLs are written into production config files
# Enforces: feedback_never_localhost_in_prod.md "Never use localhost URLs in production config"
...
# --- Emit warning ---
if [ -n "$ISSUES" ]; then
  HOOK_DECISION="warn" HOOK_REASON="localhost URLs in $BASENAME"
  SEVERITY="WARNING"
  ...
  record_hook_outcome "detect-localhost-in-config" "PostToolUse" "warn" "" "" "" "$MODEL_FAMILY"
  exit 1
fi

record_hook_outcome "detect-localhost-in-config" "PostToolUse" "allow" "" "" "" "$MODEL_FAMILY"
exit 0
