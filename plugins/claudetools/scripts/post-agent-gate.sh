...
run_validator() {
  ...
  record_hook_outcome "$name" "PostToolUse" "$decision" "Agent" "" "" "$MODEL_FAMILY"
  ...
  emit_validator_event "post-agent-gate" "$name" "$decision" "$_duration_ms" "$output" 2>/dev/null || true
}
