...
run_validator() {
  ...
  record_hook_outcome "$name" "TaskCompleted" "$decision" "" "" "" "$MODEL_FAMILY"
  ...
  emit_validator_event "task-completion-gate" "$name" "$decision" "$_duration_ms" "$output" 2>/dev/null || true
}
