#!/usr/bin/env bash
# Validator: negative pattern learner — extracts mistakes and corrections from session
...
run_learn_negatives() {
  ...
  local LOG_FILE="${CLAUDE_PLUGIN_ROOT:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)}/logs/hooks.log"
  local HOOK_BLOCKS=""
  if [[ -f "$LOG_FILE" ]]; then
  ...
  local TOTAL_CONTENT="${USER_MSGS}${ERRORS}${CORRECTIONS}"
...
- Skip things already captured in CLAUDE.md or existing memory files
  ...
  local CREATED=0
  local TYPE SLUG DESCRIPTION WHY HOW_TO_APPLY
