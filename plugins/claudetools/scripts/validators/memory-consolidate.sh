#!/usr/bin/env bash
# Validator: memory consolidation — sync files ↔ SQLite, decay, prune, regenerate MEMORY.md
...
run_memory_consolidate() {
  ...
  for mdfile in "$MEMORY_DIR"/*.md; do
    ...
    local MEM_NAME="" MEM_DESC="" MEM_TYPE="" BODY=""
    local line
...
${line}" || BODY="$line"
  ...
  local fpath fname fdesc ftype fscore local_name desc_part
  {
