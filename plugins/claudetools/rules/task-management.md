...
- Use `/managing-tasks restore` to sync TodoWrite display with persisted state.
...
- Use `/managing-tasks decompose` for tasks with 3+ distinct steps.

### On session end or before context compaction
- Run `/managing-tasks handoff` to update progress.md with session summary.
