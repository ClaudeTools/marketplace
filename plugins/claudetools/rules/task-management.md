...
- Check `.tasks/tasks.json` for current task state.
- Use `/tasks restore` to sync TodoWrite display with persisted state.
...
- Use `/tasks decompose` for tasks with 3+ distinct steps.

### On session end or before context compaction
- Run `/tasks handoff` to update progress.md with session summary.
