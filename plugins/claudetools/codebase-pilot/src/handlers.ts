...
export function handleDeadCode(): string {
  const db = getDatabase();
  if (!db) return dbErrorMessage();
  ...
  const lines: string[] = [`${dead.length} exported symbol(s) never imported:`, ""];
  let currentFile = "";
  for (const d of dead) {
...
export function handleChangeImpact(args: { symbol: string }): string {
  const db = getDatabase();
  if (!db) return dbErrorMessage();
  ...
  const symbolRow = db
    ...
    .get(args.symbol) as { name: string; path: string } | undefined;
  ...
  const importers = db
    .prepare(
      `SELECT DISTINCT f.path, i.symbols
       FROM imports i JOIN files f ON i.file_id = f.id
       WHERE i.source LIKE ? ESCAPE '\\'
          OR i.source LIKE ? ESCAPE '\\'
       ORDER BY f.path`
    )
    .all(`%${safeBasename}`, `%/${escapeLike(basename)}`) as { path: string; symbols: string | null }[];
  ...
  for (const f of directImporters) lines.push(`  ${f}`);
...
export function handleContextBudget(): string {
  const db = getDatabase();
  if (!db) return dbErrorMessage();
  ...
  const results = db
    .prepare(
      `SELECT i.source, COUNT(*) as import_count
       FROM imports i
       GROUP BY i.source
       ORDER BY import_count DESC
       LIMIT 20`
    )
    .all() as { source: string; import_count: number }[];
  ...
  const lines: string[] = ["Most-imported sources (read these first):", ""];
  for (const r of results) {
...
export function handleApiSurface(): string {
  const db = getDatabase();
  if (!db) return dbErrorMessage();
  ...
  const results = db
    .prepare(
      `SELECT s.name, s.kind, s.signature, f.path, s.line
       FROM symbols s JOIN files f ON s.file_id = f.id
       WHERE s.exported = 1
       ORDER BY f.path, s.line`
    )
    .all() as { name: string; kind: string; signature: string | null; path: string; line: number }[];
  ...
  const lines: string[] = [`${results.length} exported symbol(s):`, ""];
  let currentFile = "";
  for (const r of results) {
...
export function handleCircularDeps(): string {
  const db = getDatabase();
  if (!db) return dbErrorMessage();
  ...
  const allImports = db
    ...
    .all() as { path: string; source: string }[];
  ...
  const lines: string[] = [`${cycles.length} circular import chain(s):`, ""];
...
// --- Doctor: health check ---
export function handleDoctor(): string {
  const lines: string[] = ["codebase-pilot doctor", ""];
    ...
    try {
      ...
      if (db) {
        ...
        const staleFiles = db
          ...
          .all() as { path: string; modified_at: number }[];
        ...
        for (const f of staleFiles) {
          try {
            ...
            if (fileMtime > f.modified_at) staleCount++;
          } catch { /* file may be deleted */ }
        }

        if (staleCount > 0) {
          lines.push(
            `  [WARN] ${staleCount} of ${staleFiles.length} sampled files modified since last index`
          );
      ...
      lines.push(
        `  [WARN] Could not check freshness: ${err instanceof Error ? err.message : err}`
      );
    }
  } else {
