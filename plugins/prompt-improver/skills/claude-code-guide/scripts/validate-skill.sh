...
if [ -n "$NAME" ]; then
  ...
  if [ "$NAME" = "$DIRNAME" ]; then
    ...
    fail "name '$NAME' does not match directory name '$DIRNAME'"
  fi
else
...
if [ -n "$DESC" ]; then
  ...
  if [ "$DESC_LEN" -lt 50 ]; then
    warn "description is short ($DESC_LEN chars) — aim for 80-200 for better trigger accuracy"
  fi
  # Check for action verbs
  if echo "$DESC" | grep -qiE '\buse when\b|\buse for\b|\binvoke\b|\btrigger\b'; then
    pass "description includes trigger context (Use when/Use for)"
  else
    warn "description lacks trigger context — add 'Use when...' for better skill selection"
  fi
else
...
if [ "$LINE_COUNT" -le 500 ]; then
  ...
  fail "SKILL.md is $LINE_COUNT lines — exceeds 500-line limit. Move content to references/"
fi
...
if [ -d "$SKILL_DIR/references" ]; then
  ...
  pass "references/ directory exists ($REF_COUNT files)"
  # Check reference sizes
  while IFS= read -r ref; do
    REF_LINES=$(wc -l < "$ref")
    REF_NAME=$(basename "$ref")
    if [ "$REF_LINES" -gt 400 ]; then
      warn "$REF_NAME is $REF_LINES lines — consider splitting (target: 150-400)"
    fi
  done < <(find "$SKILL_DIR/references" -name "*.md" 2>/dev/null)
else
  if [ "$LINE_COUNT" -gt 200 ]; then
    warn "no references/ directory — consider splitting content for skills over 200 lines"
  fi
fi

if [ -d "$SKILL_DIR/scripts" ]; then
  SCRIPT_COUNT=$(find "$SKILL_DIR/scripts" -name "*.sh" 2>/dev/null | wc -l)
  pass "scripts/ directory exists ($SCRIPT_COUNT files)"
  # Syntax check all scripts
  while IFS= read -r script; do
    SCRIPT_NAME=$(basename "$script")
    if bash -n "$script" 2>/dev/null; then
      pass "$SCRIPT_NAME passes bash -n"
    else
      fail "$SCRIPT_NAME fails bash -n syntax check"
    fi
...
if [ -d "$SKILL_DIR/references" ] && [ "$REF_COUNT" -gt 0 ]; then
  ...
  if [ "$REFS_MENTIONED" -ge "$REF_COUNT" ]; then
    pass "all $REF_COUNT reference files are mentioned in SKILL.md"
  else
    warn "only $REFS_MENTIONED of $REF_COUNT reference files mentioned in SKILL.md"
  fi
fi

# Check for verification/checklist section
if grep -qiE 'verification|checklist' "$SKILL_MD"; then
  pass "verification/checklist section found"
else
  warn "no verification or checklist section — add one to guide completion checks"
fi

# Check for gotchas section
if grep -qiE 'gotcha|common mistake|pitfall|watch out' "$SKILL_MD"; then
  pass "gotchas/pitfalls section found"
else
  warn "no gotchas section — non-obvious facts prevent real mistakes"
fi

# Check for examples
EXAMPLE_COUNT=$(grep -ciE 'example|```' "$SKILL_MD" || true)
if [ "$EXAMPLE_COUNT" -ge 2 ]; then
  pass "examples found ($EXAMPLE_COUNT occurrences)"
else
  warn "few or no examples — examples are the strongest steering signal"
fi
...
while IFS= read -r ref_path; do
  ...
  if [ -f "$FULL_PATH" ]; then
    ...
    fail "referenced file missing: $ref_path"
  fi
